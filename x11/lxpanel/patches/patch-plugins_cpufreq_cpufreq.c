Index: plugins/cpufreq/cpufreq.c
--- plugins/cpufreq/cpufreq.c.orig
...
+#if defined(__linux__)
 ...
 get_cur_governor(cpufreq *cf){
     FILE *fp;
@@ -99,99 +108,7 @@ get_cur_freq(cpufreq *cf){
     }
 }
...
@@ -227,74 +144,54 @@ get_cpus(cpufreq *cf)
     }
...
-
 ...
 {
@@ -318,12 +215,18 @@ _update_tooltip(cpufreq *cf)
     char *tooltip;
...
+#endif
     ...
     g_free(tooltip);
     RET(TRUE);
@@ -344,8 +247,12 @@ static GtkWidget *cpufreq_constructor(LXPanel *panel, 
     ENTER;
     cf = g_new0(cpufreq, 1);
...
@@ -365,30 +272,14 @@ static GtkWidget *cpufreq_constructor(LXPanel *panel, 
     RET(cf->main);
 }
...
@@ -399,7 +290,11 @@ FM_DEFINE_MODULE(lxpanel_gtk, cpufreq)
 /* Plugin descriptor. */
